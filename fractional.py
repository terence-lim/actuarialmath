...
class Fractional(Lifetime):
    ...
    def l_r(self, x: int, s: int = 0, r: float = 0.) -> float:
        """Lifes at fractional age: l_[x]+s+r"""
        s += math.floor(r)  # interpolate lifes between consecutive integer ages
        r -= math.floor(r)
        if r == 0:
    ...
    def f_r(self, x: int, s: int = 0, r: float = 0., t: float = 0.0) -> float:
        """probability distribution function at fractional age: f_[x]+s+r (t)"""
